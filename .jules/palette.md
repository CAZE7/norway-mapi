...
## 2024-07-24 - Structured Skeleton Loaders

**Learning:** Replacing a plain text "Wetter & Nordlicht werden geladen…" loading message with purely visual skeleton layouts regresses accessibility for screen readers. An aria-live or sr-only fallback is required to maintain the loading state for non-sighted users.
**Action:** Always include sr-only text or proper ARIA live regions when replacing text loaders with visual skeletons.
